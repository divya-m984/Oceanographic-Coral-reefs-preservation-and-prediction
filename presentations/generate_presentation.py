...
def add_footer(slide, slide_num: int):
    ...
    add_shape(
        slide, left=Inches(0), top=Inches(7.18), width=SLIDE_W, height=Inches(0.04), fill=C_TEAL
    )
    # Footer text left
    add_tb(
        slide,
...
def add_accent_line(slide, color=C_TEAL):
    ...
    add_shape(
        slide, left=Inches(0), top=Inches(1.12), width=SLIDE_W, height=Inches(0.03), fill=color
    )
...
def add_slide_title(slide, title: str, subtitle: str = ""):
    ...
    # Title background
    add_shape(
        slide, left=Inches(0), top=Inches(0), width=SLIDE_W, height=Inches(1.12), fill=C_NAVY2
    )
    # Title text
    add_tb(
        slide,
...
def make_model_comparison_chart(metrics: dict, task_label: str) -> bytes:
    ...
    for i, (model, offset) in enumerate(zip(models, offsets)):
        ...
        rects = ax.bar(
            x + offset, vals, bar_w - 0.02, color=bar_colors[i], zorder=3, edgecolor="none"
        )
    ...
    ax.set_yticklabels([f"{v:.2f}" for v in np.arange(0.72, 0.89, 0.02)], color=M_MUTED, fontsize=8)
...
def make_preprocessing_diagram() -> bytes:
    ...
    for i in range(len(top_boxes) - 1):
        ...
        ax.annotate(
            "", xy=(x1, y), xytext=(x0, y), arrowprops=dict(arrowstyle="-|>", color=M_MUTED, lw=1.5)
        )
    ...
    ax.annotate(
        "",
        xy=(2.5, 1.1 + bh2),
        xytext=(2.5, top_boxes[0][1]),
        arrowprops=dict(arrowstyle="-|>", color="#4A6A8A", lw=1.2, linestyle="dashed"),
    )
    ax.annotate(
        "",
        xy=(5.5, 1.1 + bh2),
        xytext=(5.5, top_boxes[0][1]),
        arrowprops=dict(arrowstyle="-|>", color="#4A6A8A", lw=1.2, linestyle="dashed"),
    )
    ...
    for i, (label, color) in enumerate(stages):
        ...
        if i < n - 1:
            ...
            ax.annotate(
                ...
                xy=(x1, y),
                xytext=(x0, y),
                arrowprops=dict(arrowstyle="-|>", color=M_MUTED, lw=1.8),
            )
...
def make_dataset_summary_chart() -> bytes:
    ...
    bars2 = ax2.bar(
        rest_labels, rest_counts, color=rest_colors, edgecolor=M_NAVY3, linewidth=0.5, zorder=3
    )
    ...
    fig.suptitle(
        "Class Distribution  (15,000 synthetic observations — approximate from test-split proportions)",
        color=M_MUTED,
        fontsize=7.5,
        y=0.02,
    )
...
def slide_01_title(prs):
    ...
    add_shape(
        slide, left=Inches(0), top=Inches(3.9), width=SLIDE_W, height=Inches(3.6), fill=C_NAVY2
    )

    # Decorative teal band
    add_shape(
        slide, left=Inches(0), top=Inches(3.88), width=SLIDE_W, height=Inches(0.05), fill=C_TEAL
    )

    # Cyan accent vertical bar
    add_shape(slide, left=Inches(0), top=Inches(0), width=Inches(0.12), height=SLIDE_H, fill=C_TEAL)
...
def slide_02_problem(prs):
    ...
    set_bg(slide, C_NAVY)
    add_slide_title(
        slide, "Problem and ML Objectives", "Why machine learning for coral reef monitoring?"
    )
    add_footer(slide, 2)
...
def slide_03_data_collection(prs):
    ...
    set_bg(slide, C_NAVY)
    add_slide_title(
        slide, "Data Collection Strategy", "Current prototype vs. proposed future deployment"
    )
    add_footer(slide, 3)
...
def slide_04_features(prs):
    ...
    groups = [
        ...
        },
        {
            "title": "Target Labels",
            "color": C_CORAL,
            "features": ["reef_health (4 classes)", "restoration_suitability (3 classes)"],
    ...
    for grp, (lx, ty, bw, bh) in zip(groups, positions):
        ...
        add_shape(
            slide,
            left=Inches(lx + 0.06),
            top=Inches(ty + bh - 0.52),
            width=Inches(bw - 0.12),
            height=Inches(0.44),
            fill=C_HIGHLIGHT,
        )
        add_tb(
            slide,
            left=Inches(lx + 0.08),
            top=Inches(ty + bh - 0.50),
            width=Inches(bw - 0.14),
            height=Inches(0.42),
            text=grp["note"],
            size=Pt(8.2),
            italic=True,
...
def slide_06_validation(prs):
    ...
    add_image_bytes(
        slide, val_img, left=Inches(0.25), top=Inches(1.22), width=Inches(12.85), height=Inches(3.5)
    )
    ...
    for i, (check, desc) in enumerate(checks):
        ...
        add_tb(
            slide,
            left=Inches(col_widths[0] + 0.55),
            top=Inches(y + 0.02),
            width=Inches(col_widths[1]),
            height=Inches(0.26),
            text=desc,
            size=Pt(10),
            color=C_TEXT,
...
def slide_07_preprocessing(prs):
    ...
    right_items = [
        (
            "Categorical Pipeline (1 feature: region)",
            C_TEAL,
            [
    ...
    for (title, color, lines), x_start in [(left_items[0], 0.28), (right_items[0], 6.65)]:
        add_shape(
            slide,
            left=Inches(x_start),
            top=Inches(5.15),
...
def slide_09_health_results(prs):
    ...
    chart_img = make_model_comparison_chart(
        HEALTH_METRICS, "Task: Reef Health Classification  |  4 classes  |  Test set = 3,000 rows"
    )
    add_image_bytes(
        slide,
        chart_img,
...
def slide_10_restoration_results(prs):
    ...
    add_slide_title(
        slide,
        "Restoration Suitability Model Comparison",
        "Selected model: XGBoost (highest CV Macro-F1) — SYNTHETIC DATA",
    )
    add_footer(slide, 10)

    chart_img = make_model_comparison_chart(
        RESTORATION_METRICS, "Task: Restoration Suitability  |  3 classes  |  Test set = 3,000 rows"
    )
    add_image_bytes(
        slide,
        chart_img,
...
def slide_11_reproducibility(prs):
    ...
    tools = [
        ...
        (
            "DVC",
            "Data and pipeline versioning — dvc repro reruns only changed stages (params.yaml-driven).",
        ),
        (
            "params.yaml",
            "Single source of truth for all hyperparameters, dataset size, split ratios, thresholds.",
        ),
        (
            "MLflow",
            "Experiment tracking — logs parameters, CV scores, test metrics, confusion matrix, model artifact.",
        ),
        (
            "Saved preprocessors",
        ...
        (
            "Model registry",
            "Champions registered in MLflow registry with 'champion' alias. Candidates never auto-promoted.",
        ),
        (
            "1,023 tests",
            "Full pytest suite (tests/) validates pipeline, API, drift, retraining, and governance.",
        ),
    ]
...
def slide_12_conclusions(prs):
    ...
    for label, val in results:
        ...
        add_tb(
            ...
            size=Pt(10.5),
            bold=bold,
            color=C_TEXT if not bold else C_CYAN,
            align=PP_ALIGN.RIGHT,
        )
        y += 0.30
...
def main():
    ...
    print("  All 12 slides verified")
    print("  Speaker notes present on all slides")
