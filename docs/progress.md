...
- Artifact paths in `artifacts/mlruns.db` are absolute; if the project moves again, run the path-migration script or re-run `src/models/train.py`.
- Quality gate thresholds are conservative; small-dataset test fixtures may not naturally pass them.
- No FastAPI serving, Streamlit UI, drift monitoring, or Docker deployment yet (planned for M7+).
- `mlflow.db` at project root is unused — left in place as per audit guidance (do not delete arbitrarily).

---

## Planned Next Steps (M7+)

- M7: DVC pipeline (`dvc.yaml` already scaffolded)
- M7: GitHub Actions CI/CD (`/.github/workflows/` already scaffolded)
- M7: FastAPI inference endpoint (`src/api/`)
- M7: Evidently AI drift monitoring (`src/monitoring/`)
- M7: Docker containerisation (`docker-compose.yml` already scaffolded)
