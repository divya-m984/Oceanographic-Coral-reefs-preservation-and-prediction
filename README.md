...
### Status of `data/raw/observations.csv`

`data/raw/observations.csv` is a **synthetic prototype dataset**: 15,000 rows,
21 columns, generated by `src/data/generate_data.py` with fixed seed 42
(SHA-256 `a03cb3e92ba1904ae07147da95f96aa689d092d56fc41b040b701a101ad8f458`).
Its `reef_health` and `restoration_suitability` columns are **algorithmically
generated labels**, computed as documented weighted formulas over other columns
in the same file rather than observed in the field.

**It is retained for:**

- deterministic CI and regression testing (fixed seed, fixed hash)
- MLOps pipeline demonstrations (DVC, MLflow, drift monitoring, promotion/rollback)
- dashboard and API demonstrations
- *relative* model benchmarking — comparing algorithms against one another

**It is not:**

- a real ecological validation corpus
- a source of independent biological ground truth
- appropriate evidence for any conservation, management, or policy decision

See [`docs/audits/dataset_scientific_audit_2026-08-19.md`](docs/audits/dataset_scientific_audit_2026-08-19.md)
for the full inventory and scientific audit of this dataset.
...
Quality gates require minimum CV macro-F1 of 0.70 (health) and 0.73 (restoration) for candidate promotion.

> ### ⚠️ Scientific disclosure — how to read these numbers
>
> These metrics were measured on a **synthetic prototype dataset** whose labels are
> **algorithmically generated** from variables that are themselves supplied to the
> models as predictors. This is **label-construction leakage / circular supervision**:
> the target column is correctly excluded from the feature matrix, but the variables
> that construct it are not. Several engineered features additionally reproduce
> components of the label-generating formula exactly (**engineered proxy leakage**).
>
> The figures above therefore measure **recovery of the synthetic generation rules and
> MLOps pipeline behaviour — not validated real-world coral-reef prediction accuracy**.
> There is **no independent biological ground truth** anywhere in this project, and
> these models have **not** been externally validated against real reef observations.
>
> **Audit diagnostic.** A closed-form re-computation of the generator's own scoring
> formula — with its Gaussian noise term removed and no machine learning of any kind —
> reproduces the stored labels at macro-F1 ≈ 0.770 (reef health) and ≈ 0.812
> (restoration), matching or exceeding an equivalent trained model (≈ 0.760 / ≈ 0.791).
> Those four figures are **audit diagnostics only**; the canonical registered champion
> metrics are the ones tabulated above.
>
> Full analysis: [`docs/audits/dataset_scientific_audit_2026-08-19.md`](docs/audits/dataset_scientific_audit_2026-08-19.md).
...
- **Label-construction leakage / circular supervision** — Both target columns are **algorithmically generated** from variables that are then supplied to the models as predictors. The targets themselves are correctly excluded from the feature matrix, but their generating variables are not, and several engineered features reproduce components of the label formula exactly (**engineered proxy leakage**). Reported metrics therefore measure recovery of the synthetic generation rules, not real-world ecological validity. Documented in full in [`docs/audits/dataset_scientific_audit_2026-08-19.md`](docs/audits/dataset_scientific_audit_2026-08-19.md).
- **No independent biological ground truth** — Nothing in this project contains a reef condition that was actually observed. `coral_cover_percentage`, `bleaching_percentage`, and `disease_percentage` are generated variables, and in a real deployment would be survey ground truth rather than model inputs.
